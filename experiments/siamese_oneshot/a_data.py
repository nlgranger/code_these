...
def prepare():
    ...
    # Load raw data
    # signers = dataset.rec_info['signer'][:len(dataset)]
    # labels = dataset.rec_info['label'][:len(dataset)]
    # glosses = [dataset.glosses(i) for i in range(len(dataset))]
    ...
    # Organize splits
    train_subset = dataset.default_split()
    # shuffled_labels = np.random.permutation(np.unique(labels))
